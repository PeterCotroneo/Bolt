"""Bolt — main plugin class.

A dock panel to pick a source and an area (current map view or a drawn box),
start/stop the live lightning feed, and watch strikes flash and fade. The feed
runs inside the provider (WebSocket I/O off the GUI thread); the map is updated
in batches by a timer so QGIS stays responsive.
"""
...
class BoltPlugin:
    ...
    def initGui(self):
        ...
        self.action = QAction(icon, "Bolt", self.iface.mainWindow())
        self.action.setCheckable(True)
        self.action.toggled.connect(self._toggle_dock)
        self.iface.addToolBarIcon(self.action)
        self.iface.addPluginToMenu("Bolt", self.action)
    ...
    def unload(self):
        ...
        if self.action is not None:
            ...
            self.iface.removePluginMenu("Bolt", self.action)
            self.action = None
    ...
    def _build_dock(self):
        dock = QDockWidget("Bolt", self.iface.mainWindow())
        ...
        intro = QLabel(
            "Watch live lightning. Pick an area, then Start — strikes flash onto "
            "the map as they happen and fade over a minute."
        )
    ...
    def _start(self):
        ...
        if not is_configured(cls):
            bar.pushWarning("Bolt", f"Configure {cls.label} first.")
            ProviderConfigDialog(cls, self.iface.mainWindow()).exec()
            if not is_configured(cls):
                return
        bbox = self._bbox_wgs84()
        if bbox is None:
            bar.pushWarning("Bolt", "Draw an area on the map first.")
            return

        dbg(f"Started tracking with {cls.label}.")
        self.store.ensure_layer()
        self.provider = cls(load_settings(cls))
        self.provider.strikes_update.connect(self.store.ingest_many)
    ...
    def _stop(self):
        ...
        if self.provider is not None:
            try:
                ...
                QgsMessageLog.logMessage(f"stop: {exc}", "Bolt", Qgis.MessageLevel.Warning)
            self.provider = None
        self.store.flush()
        self._running = False
    ...
    def _dismiss_stale_highlights(self):
        """Hide our layer's Identify highlights once strikes have expired, so a
        red highlight doesn't linger where a strike used to be. Other layers'
    ...
    def _update_status(self):
        if self.lbl_status is None:
            return
        self.lbl_status.setText(f"{self._last_status} · {self.store.count()} strikes in view")
